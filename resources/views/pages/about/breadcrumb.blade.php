{{-- Breadcrumb Section --}}
@php
    $breadcrumbSection = $aboutPage->sections['breadcrumb'] ?? [];
    $isActive = $breadcrumbSection['is_active'] ?? true;
    $pageTitle = $breadcrumbSection['page_title'] ?? 'About Us';
    $bgImage = !empty($breadcrumbSection['background_image'])
        ? (str_starts_with($breadcrumbSection['background_image'], 'http')
            ? $breadcrumbSection['background_image']
            : (str_starts_with($breadcrumbSection['background_image'], 'assets/')
                ? asset($breadcrumbSection['background_image'])
                : asset('storage/' . $breadcrumbSection['background_image'])))
        : asset('assets/img/breadcrumb/breadcrumb-img.png');
@endphp
...
<section class="breadcrumb-area wa-p-relative">
    <div class="breadcrumb-bg-img wa-fix wa-img-cover">
        <img class="wa-parallax-img" src="{{ $bgImage }}" alt="">
    </div>

    <div class="container bs-container-1">
        <div class="breadcrumb-wrap">
            <h1 class="breadcrumb-title wa-split-right wa-capitalize" data-split-delay="1.1s">{{ $pageTitle }}</h1>
